...
//    ui->btnFocusTest->setEnabled(false);
    ...
    focusThread = new QThread(this);
    m_Focuser = new Focuser();
    m_Focuser->moveToThread(focusThread);
    QObject::connect(this,SIGNAL(destroyed(QObject*)),focusThread,SLOT(quit()));
    focusThread->start();

    // STarts Image grabber
    ...
    std::cout<<"Starting camerathreadviewer" <<std::endl;
    ...
    QObject::connect(ui->pbFocus,SIGNAL(clicked()),m_Focuser,SLOT(sendMessage()));
...
//    timer->start();
    ...
    std::cout<< "Creating focuser" <<std::endl;

   //m_Focuser = new Focuser(this);
    //QObject::connect(ui->zaxisMsgBtn,SIGNAL(clicked()),m_Focuser,SLOT(initiateFocus()));
...
//    QObject::connect(this,SIGNAL(doLightsCalib(uint)),m_lights,SLOT(calibrateLights(uint)));
    ...
    //QObject::connect(ui->pbZ,SIGNAL(clicked()),this,SLOT(zAxisMove()));
...
{
    std::cout<<"Light Calibration" << std::endl;
...
void CameraThreadViewerGUI::newMessage(QString type, QByteArray data)
{
    using namespace std;
    /*QMessageBox msgBox;
    msgBox.setText("The document has been modified TWO.");
    msgBox.exec();
...
void CameraThreadViewerGUI::zAxisMove(){
    QTime timer;
