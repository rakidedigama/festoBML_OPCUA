...
void CameraBufferWrapper::run()
{
    ...
    //unsigned uBayerPattern = 0;
    unsigned uBayerPattern = ini.GetSetValue("Images", "Bayer", 0, "0 = RGGB (Imperx), 1 = GRBG (Basler)");
                ...
                if (m_bSaving || m_bSaveOne)
                {
                    ...
                    else
                    {
                        PWImage8 imgRawUC(
                                    ...
                                    uWidth,
                                    uHeight);
                        rawImg.copyTo8Bit(imgRawUC.getImagePointer(),imgRawUC.m_pHeader->m_uSizeBytes);
